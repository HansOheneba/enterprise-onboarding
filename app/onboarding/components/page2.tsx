...
const RISK_TOLERANCE = [
  { 
    value: 'low', 
    label: 'Conservative', 
    description: 'I prefer stable, predictable returns even if growth is slower',
  },
  { 
    value: 'medium', 
    label: 'Balanced', 
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="space-y-6 bg-white p-6 rounded-2xl border border-black/10">
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-semibold text-neutral-900">Financial Knowledge Level</h2>
          </div>
          ...
          <div className="space-y-4">
            {FINANCIAL_KNOWLEDGE_LEVELS.map((level) => (
              <div
                ...
                onClick={() => updateData({ financialKnowledge: level.value as any })}
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-lg font-semibold text-neutral-900">{level.label}</h3>
                    </div>
                    <p className="text-sm text-neutral-700 mb-3">{level.description}</p>
                  ...
                  <div className={`h-5 w-5 rounded-full border-2 ml-4 mt-1 ${ 
                    data.financialKnowledge === level.value
        ...
        <div className="space-y-6 bg-white p-6 rounded-2xl border border-black/10">
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-semibold text-neutral-900">Financial Goals</h2>
          </div>
        ...
        <div className="space-y-6 bg-white p-6 rounded-2xl border border-black/10">
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-semibold text-neutral-900">Risk Tolerance</h2>
          </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      <h3 className="font-semibold text-neutral-900">{risk.label}</h3>
                    </div>
                    <p className="text-sm text-neutral-600">{risk.description}</p>
                  </div>
                  <div className={`h-5 w-5 rounded-full border-2 ml-4 ${
                    data.riskTolerance === risk.value
                      ? 'border-[#1B1856] bg-[#1B1856]'
                      : 'border-black/30'
                  }`} />
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-4">
            <h4 className="font-medium text-blue-800 mb-1">Understanding Risk Tolerance</h4>
            <p className="text-sm text-blue-700">
              Your risk tolerance helps us recommend suitable investment strategies. 
              Higher risk typically means higher potential returns, but also greater volatility.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6 bg-white p-6 rounded-2xl border border-black/10">
          <h2 className="text-xl font-semibold text-neutral-900">Additional Preferences</h2>
